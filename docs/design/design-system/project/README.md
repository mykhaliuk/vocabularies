...
### Accessible action surfaces (text-on-accent)
White-label buttons render on **`--primary-action` (`#CF3A60`, 4.74:1)** and **`--secondary-action` (`#1378A6`, 4.93:1)** — dedicated steps that clear **WCAG 2.1 AA** (1.4.3, ≥4.5:1) with white. The bright brand accents `--primary` (rose-500, 3.43:1) and `--secondary` (blue-500, ~2.9:1) **fail** AA with white, so they are reserved for non-text uses: washes, focus rings, `--like`, the eyebrow dot, and the FAB glyph. Action surfaces are **theme-independent** (a single `:root` definition, *not* overridden under `[data-theme="dark"]`) — the button surface *is* the colour, so it must pass in both themes. `Button variant="primary"|"blue"` consume these automatically.
