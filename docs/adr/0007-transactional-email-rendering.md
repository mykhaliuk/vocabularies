...
- The logo is the raster PWA mark (icon-192.png) embedded as an inline CID
  attachment — SVG is unreliable in Gmail/Outlook, and a remote URL proved
  fragile in VKB-69 (dev origin behind Vercel SSO, prod on a release that
  predated the asset). The base64 module `server/utils/emails/logo.ts` is
  regenerated by `icons:gen` together with the icon set.
