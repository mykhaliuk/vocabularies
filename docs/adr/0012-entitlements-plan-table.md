...
| Per-plan limits inside the pipeline                             | A plan's numbers actually differ. `PLANS` and `loadEntitlements` already carry them; what remains is passing them into `processMedia` instead of the module constants                                                                                                              |
