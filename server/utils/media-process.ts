...
const extractPeaks = async (path: string): Promise<number[] | null> => {
  ...
  try {
    const { stdout } = await execFileAsync(
      ...
      { maxBuffer: EXEC_MAX_BUFFER, encoding: 'buffer' },
    );
    pcm = stdout;
  } catch (error) {
...
const SCALE_720 =
  `scale=w='if(gt(iw,ih),-2,min(${VIDEO_TARGET_EDGE},iw))'` +
  `:h='if(gt(iw,ih),min(${VIDEO_TARGET_EDGE},ih),-2)'`;
...
export const processMedia = async (
  ...
  try {
    ...
    if (durationSec > MAX_DURATION_SEC + DURATION_TOLERANCE_SEC) {
      throw new Error(
        `[media-process] duration ${durationSec.toFixed(1)}s exceeds ${MAX_DURATION_SEC}s limit`,
      );
    }
    ...
    const peaks = await extractPeaks(originalPath);
    const peaksMs = Date.now() - peaksStart;
