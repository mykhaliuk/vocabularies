...
| M3 | Drizzle schema + migrations     | ⏳ in progress  | `m3` | 3 tables + unique index on `lower(email)` after `db:migrate`  |
| M4 | Magic-link auth E2E             | 🔲 pending     | `m4` | login → `/me` → logout works locally; expired link 401        |
...
| M9 | Multi-env runner + DB dump      | 🔲 pending     | `m9` | health passes for all 3 envs; `db:dump:dev` populates local   |
