...
const goBack = () => {
  // A word opened from the feed returns to it. A word opened cold — a
  // refresh, a pasted link — has nothing of ours behind it, so the feed is
  // the destination rather than whatever the tab held before the app.
  const previous = router.options.history.state.back;
  if (typeof previous === 'string') {
    router.back();
    return;
  }
  void navigateTo('/feed');
};
</script>
